 ...
 * @param state
 */
function authors_ChangeCountry(frm, country, state)
{
    // define this to prevent name overlapping
    var $ = jQuery;
 ...
 * @param object el form element
 */
function authors_RegistrationForm(el)
{
    if(el == null || jQuery(el).hasClass('hover')) return false;
    // define this to prevent name overlapping
    var $ = jQuery;

    ///var authors_registrationForm = $(el).closest('form');
                ...
                try{
                    ...
                    if(obj.status == '1'){
                        ...
                        if(obj.error != null && obj.error != ''){
                            $('#messageSuccess p.alert-warning').show().text(obj.error);
                        }
                    }else{
                        authors_RaiseError(el, obj.error, obj.error_field);
                    }
                }catch(err){
                    authors_RaiseError(el, err.message);
                }
            }
        });
    }
 ...
 * @param object el form element
 */
function authors_RestorePasswordForm(el)
{
    if(el == null) return false;
    // define this to prevent name overlapping
    var $ = jQuery;
 ...
 * @param errorField
 */
function authors_RaiseError(el, errorDescription, errorField)
{
    // define this to prevent name overlapping
    var $ = jQuery;
