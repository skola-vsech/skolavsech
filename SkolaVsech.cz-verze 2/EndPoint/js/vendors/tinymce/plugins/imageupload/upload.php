...
}else if(CAuth::isLoggedInAsAdmin() && !Admins::hasPrivilege($privilege_category, array('add', 'edit'))){
    echo '<html>';
    echo '<head><title>Access Error</title></head>';
    echo '<body>';
...
}else if(CAuth::isLoggedInAs('author') && !Modules::model()->exists("code = 'mblog' AND is_installed = 1")){
    echo '<html>';
    echo '<head><title>Access Error</title></head>';
    echo '<body>';
    echo A::t('app', 'You have no privileges to view this page');
